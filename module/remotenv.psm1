...
.\remotenv.ps1
  ...
  cmdlets:          build powershell cmdlets
    ...
    $Content = Get-Content "$($drive)/Windows/System32/WindowsPowerShell/v1.0/Modules/remotenv/config.txt"
    ...
    if($setting -ne "docker" -And $setting -ne "remote machine"){
      ...
      if($mode -ne "build" -And $mode -ne "reset"){
        ...
        try{
          ...
          Invoke-Command {docker exec -it $docker_name sh -c "cp /$($project)/bash/monitor.service /lib/systemd/system/monitor.sservice"}
